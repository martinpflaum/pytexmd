#%%
from .core import Element,Undefined,GuardianSearch,ReplaceSearch,Label,JunkSearch,Document
...
class TexArray(Element):
    ...
    def position(string: str) -> int:
        if "\\begin{array}" in string:
            return splitting.position_of(string,"\\begin{array}")
        else:
            return -1
        
    @staticmethod
    def split_and_create(string: str, parent: Element) -> tuple:
        pre,content,post = splitting.begin_end_split(string,"\\begin{array}","\\end{array}")
...
class BeginEquationEnumElement(Element):
    def __init__(self, modifiable_content: str, parent: Element):
        ...
        search_func = lambda instance : misc.has_value_equal(instance,"theorem_env_name",number_within_equation)
...
class BeginAlignSearcher():
    ...
    def position(self, string: str) -> int:
        return splitting.position_of(string,self.begin)
        
    def split_and_create(self, string: str, parent: Element) -> tuple:
        pre,content,post = splitting.begin_end_split(string,self.begin,self.end)
...
class BeginEquationEnumSearcher():
    def __init__(self, name: str):
        ...
        self.name = name
    def position(self, string: str) -> int:
        return splitting.position_of(string,"\\begin{"+self.name+ "}")
    
    def split_and_create(self, string: str, parent: Element) -> tuple:
        pre,content,post = splitting.begin_end_split(string,"\\begin{"+self.name+"}","\\end{"+self.name+"}")
        out = BeginEquationEnumElement(content,parent)
        out = apply_latex_protection(out)
...
class InlineLatex(Element):
    ...
    @staticmethod
    def position(string: str) -> int:
        if splitting.position_of(string,"$",save_split=False) == splitting.position_of(string,"$$",save_split=False):
            return -1
        else:
            return splitting.position_of(string,"$",save_split=False)
        
    @staticmethod
    def split_and_create(string: str, parent: Element) -> tuple:
        pre,modifiable_content = splitting.split_on_next(string,"$",save_split=False)
        in_outer_dollar = ""
        post = "" 
        content = ""

        while True:
            pending_pre_end,post = splitting.split_on_next(modifiable_content,"$",save_split=False)
            if not "\\text" in pending_pre_end:
                content = in_outer_dollar + pending_pre_end
                break
            content_unknown,tmp_post = splitting.split_on_next(modifiable_content,"\\text",save_split=False)
            brace_content,modifiable_content = splitting.split_on_first_brace(tmp_post)
...
class LatexText(Element):
    ...
    @staticmethod
    def position(string: str) -> int:
        return splitting.position_of(string,"\\text")

    @staticmethod
    def split_and_create(string: str, parent: Element) -> tuple:
        pre,post = splitting.split_on_next(string,"\\text")
        content,post = splitting.split_on_first_brace(post)
...
class Cases(Element):
    ...
    def position(string: str) -> int:
        if "\\begin{cases}" in string:
            return splitting.position_of(string,"\\begin{cases}")
        else:
            return -1
        
    @staticmethod
    def split_and_create(string: str, parent: Element) -> tuple:
        pre,content,post = splitting.begin_end_split(string,"\\begin{cases}","\\end{cases}")
...
def get_all_latex_searchers() -> list:
    #The derivatives are 
    multiline = ["split", "multline","align","breqn","equation"]
...
class DoubleDolarLatex(Element):
    ...
    def split_and_create(string: str, parent: Element) -> tuple:
        pre,modifiable_content = splitting.split_on_next(string,"$$",save_split=False)
        content,post = splitting.split_on_next(modifiable_content,"$$",save_split=False)  
        out = Undefined("<br><br><span class='display'>" + content + "</span><br>",parent)
        out = apply_latex_protection(out)
        out.expand([ReplaceSearch("\\\\","</span><br><br><span class='display'>"),JunkSearch("&")])
        return pre,out,post
